...
public class MoHinhDao {
        ...
        getAllMoHinh(new OnGetListMoHinhSuccess() {
            ...
            public void onGetSuccess(List<MoHinh> moHinhList) {
                ...
                if (moHinhList.size() > 0) {
                    mamh = moHinhList.get(moHinhList.size() -1).getMaMh() +1;
                }
                moHinh.setMaMh(mamh);
                    ...
                    public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                        storageReference.child("img_mo_hinh_" + moHinh.getMaMh()).getDownloadUrl()
                                .addOnSuccessListener(new OnSuccessListener<Uri>() {
                                    ...
                                    public void onSuccess(Uri uri) {
                                        moHinh.setImgUri(uri.toString());
                                        dbReference.child("mo_hinh" + moHinh.getMaMh()).setValue(moHinh);
                                    }
                                });
                    }
                });
    ...
    public void deleteMoHinh(int mamh) {
        progressDialog.show();
        dbReference.child("mo_hinh_" + mamh).removeValue();
        storageReference.child("img_mo_hinh_" + mamh).delete().addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
